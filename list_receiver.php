...
$(document).ready(function(){
	...
	$('#Reflectortable').on('shown.bs.collapse', function () {
	    update_icons_expand();
	});

	$('#Reflectortable').on('hidden.bs.collapse', function () {
	   update_icons_expand();
	});
...
function update_icons_expand()
{
	...
	$( "#Reflectortable tr" ).each(function( i ) {
    ...
    for(var qth in data.nodes[k].qth)
    {
       	...
       	if(rx_active == true && rx_sql == true)
    	{
    		...
    		if(mqtt_station_array[k]  && mqtt_station_array[k]["RSSI"] != "-200")
    		{
    			...
    			$("#receiver_"+k).html(Math.round(mqtt_station_array[k]["RSSI"])+" dBm<br/>"+qth_name_location);
    		}
    		else
    		{
    			$("#receiver_"+k).html(qth_name_location);
    		}
...
function update_bar_rssi(id,value,siglev,k)
{
    ...
    function seek() {
    	  ...
    	  table = document.getElementById("Reflectortable");
